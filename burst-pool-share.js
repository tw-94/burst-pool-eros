...
var storedTotalCumulativeShares = 1;

var poolCapacity = {};
var poolCapacityTotal = 0;
var poolCapacityAccounts = {};
var lastCapacityCleaning = 50;
var poolAccountMiner = {};
 	...
 	console.log("Cleaning pool capacity");
        ...
        for (var accountId in poolCapacity) {
            ...
            for (var ip in poolCapacity[accountId]) {
                ...
                if (ipdata.height >= height - 90) {
		    if(parseFloat(ipdata.capacity) >= 100000) {
			cap = parseFloat(ipdata.capacity);
			} else {
			cap = parseFloat(ipdata.capacity);
			}
                    poolCapacityAccounts[accountId] = cap;
                    poolAccountMiner[accountId] = ipdata.miner;
...
function AddCapacity(accountId, ip, capacity, height, miner) {
    ...
    poolCapacity[accountId][ip].miner = miner;
}
...
/** Set TB for each Miner
*/
var test;
var capAccount;
var caps = [];
var accounts = [];
PoolShare.prototype.setTB = function(cap, accountId){

     var capString = "~" + (cap / 1024).toFixed(2) + "TB";
     capAccount = accountId;
     test = capString;

     var i = accounts.indexOf(accountId);

       if(accounts[i] == accountId) {
         accounts[i] = accountId;
         caps[i] = capString;
       } else if(!caps[i]) {
         caps.unshift(capString);
         accounts.unshift(accountId);
       }
...
PoolShare.prototype.getCumulativeShares = function () {
    ...
    for (var accountId in blockShareList) {
        ...
        shareList.forEach(function (share) {
            ...
            if (cumulativeShare.hasOwnProperty(accountId)) {
                ...
                cumulativeShare[accountId].roundCount++;
				  ...
				  var i = accounts.indexOf(accountId);
                  if(accounts[i] == accountId) {
                    cumulativeShare[accountId].pending = caps[i];
                  }
                if (share.deadline > 0) {
        ...
        if (poolCapacityAccounts.hasOwnProperty(accountId)) {
          cumulativeShare[accountId].capacity = parseFloat(poolCapacityAccounts[accountId] / 1024).toFixed(2);
          cumulativeShare[accountId].miner = poolAccountMiner[accountId];
        }
        else {
          cumulativeShare[accountId].capacity = 0;
          cumulativeShare[accountId].miner = "-";
        }
    }
...
module.exports = {
	...
	setTB : function(accountId, cap){
        poolShare.setTB(accountId, cap);
    },
